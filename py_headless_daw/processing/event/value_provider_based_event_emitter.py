...
class ValueProviderBasedEventEmitter(ProcessingStrategy):
    ...
    def __init__(self, provider: ValueProvider, transformer: Callable[[float], Event],
                 sampling_period=DEFAULT_SAMPLING_PERIOD):
        super().__init__()
        self._provider: ValueProvider = provider
        self._transformer_function: Callable[[float], Event] = transformer
    ...
    def render(self, interval: TimeInterval, stream_inputs: List[np.ndarray], stream_outputs: List[np.ndarray],
        ...
        event: Event = self._transformer_function(value)
